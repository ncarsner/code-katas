...
def lottery_analysis(n: int, r: int = 5) -> dict:
    ...
    odd_even_dist = Counter((x["odd_count"], x["even_count"]) for x in results)
    ...
    return {
        ...
        "odd_even_distribution": dict(sorted(odd_even_dist.items())),
        "prime_distribution": dict(sorted(prime_dist.items())),
...
if __name__ == "__main__":
    ...
    random_sample = (
        True  # Set to False to see the first 10 combinations instead of random samples
    )
    ...
    print("\nSum distribution (10 most common, ~normal dist):")
    top_sums = dict(sorted(Counter(output["sum_distribution"]).most_common(10)))
    for k, v in top_sums.items():
        print(f"  Sum {k}: {v:,}")
