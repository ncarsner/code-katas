    ...
    for i in range(2):
        for j in range(2):
            for k in range(2):
                for m in range(2):
                    variation = (
                        f"{parts[0] if i == 0 else single_digit_parts[0]}"
                        f"{parts[1] if j == 0 else single_digit_parts[1]}"
                        f"{parts[2] if k == 0 else single_digit_parts[2]}"
                        f"{parts[3] if m == 0 else single_digit_parts[3]}"
                    )
...
def parse_variations(variations):
    ...
    for variation in variations:
        parsed_variation = punctuation.join(
            [variation[i : i + 2] for i in range(0, len(variation), 2)]
        )
