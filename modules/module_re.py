...
emails = re.findall(
    r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b", text_with_emails
)
...
def format_currency(text):
    ...
    return re.sub(
        pattern, lambda m: f"${(n := m[1])} {'dollar' if n == '1' else 'dollars'}", text
    )
...
if __name__ == "__main__":
    ...
    formatted_matches = [
        {"word": match[0], "percentage": f"{match[1] * 100:.1f}%"} for match in matches
    ]
    # print(json.dumps(formatted_matches, indent=4))

    # Keep backreferences in a replacement
    text_with_backreferences = (
        "The price is USD 100 and the discount is USD 20 after the USD 15 fine."
    )
